...
// === Load Python Module via jsDelivr (CORS-FREE) ===
async function loadPyodideAndModule() {
    ...
    try {
        ...
        // === USE jsDelivr: CORS-FREE, FAST, RELIABLE ===
        const moduleUrl = "https://cdn.jsdelivr.net/gh/YOUR-USERNAME/portfolio-var-calculator@main/var_calculator.py";
        console.log("Loading var_calculator.py from jsDelivr...");
        const response = await fetch(moduleUrl);
        if (!response.ok) throw new Error(`Failed to load: ${response.status}`);
        const pyCode = await response.text();
        pyodide.runPython(pyCode);
        console.log("var_calculator.py loaded successfully");
...
// === [Rest of the code: Search, Portfolio, VaR, Chart] ===
// (Same as before — only the loading part changed)
...
function addToPortfolio(symbol, name) {
    if (portfolio.length >= 5) { alert("Max 5 securities."); return; }
    const weight = parseFloat(document.getElementById("weightInput").value);
    if (isNaN(weight) || weight <= 0 || weight > 100) { alert("Weight 1–100%"); return; }
    portfolio.push({ symbol, name, weight });
    updatePortfolioTable();
    searchResults.innerHTML = ""; searchInput.value = "";
}

function removeFromPortfolio(i) { portfolio.splice(i, 1); updatePortfolioTable(); }
...
calculateVarBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        const resultJson = await window.pyodide.runPythonAsync(`
            ...
            json.dumps(result)
        `);
        displayVaRResults(JSON.parse(resultJson));
    } catch (err) {
...
function displayVaRResults(data) {
    ...
    for (const [sym, vals] of Object.entries(data)) {
        html += `<tr><td><b>${sym}</b></td><td>${vals.Normal95}</td><td>${vals.Normal99}</td><td>${vals.Hist95}</td><td>${vals.Hist99}</td><td>${vals.MC95}</td><td>${vals.MC99}</td><td>${vals.CF95}</td><td>${vals.CF99}</td><td>${(vals.ExpReturn*100).toFixed(2)}%</td></tr>`;
    }
    html += `</table>`;
...
function createRiskReturnChart(data) {
    ...
    for (const [sym, vals] of Object.entries(data)) {
        if (sym === "Portfolio") { labels.push("PORTFOLIO"); risks.push(-vals.Normal95 * 100); returns.push(vals.ExpReturn * 100); colors.push('rgba(255,0,0,0.8)'); sizes.push(120); }
        else { labels.push(sym); risks.push(-vals.Normal95 * 100); returns.push(vals.ExpReturn * 100); colors.push('rgba(245,166,35,0.7)'); sizes.push(80); }
    }
    if (riskChart) riskChart.destroy();
    riskChart = new Chart(ctx, { type: 'scatter', data: { datasets: [{ data: labels.map((l,i)=>({x:risks[i],y:returns[i],label:l})), backgroundColor: colors, borderColor: colors.map(c=>c.replace('0.7','1').replace('0.8','1')), borderWidth: 2, pointRadius: sizes.map(s=>s/10) }] }, options: { responsive: true, plugins: { tooltip: { callbacks: { label: ctx => `${ctx.raw.label}: Risk ${ctx.raw.x.toFixed(3)}%, Return ${ctx.raw.y.toFixed(2)}%` } }, legend: { display: false } }, scales: { x: { title: { display: true, text: '1-Day VaR 95% (Loss %)', color: '#fff' }, ticks: { color: '#fff' }, grid: { color: '#333' } }, y: { title: { display: true, text: 'Expected Annual Return (%)', color: '#fff' }, ticks: { color: '#fff' }, grid: { color: '#333' } } } } });
}
...
showPricesBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        let html = `<table border="1"><tr><th>Date</th>`; symbols.forEach(s => html += `<th>${s}</th>`); html += `</tr>`;
        df.data.forEach((row, i) => { html += `<tr><td>${df.index[i]}</td>`; row.forEach(v => html += `<td>${v !== null ? v : 'N/A'}</td>`); html += `</tr>`; });
        html += `</table>`;
        priceData.innerHTML = html;
    } catch (err) { priceData.innerHTML = "<p style='color:red;'>Failed.</p>"; }
});
