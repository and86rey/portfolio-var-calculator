...
function displaySearchResult(stock) {
    searchResults.innerHTML = "<p><b>" + stock.name + "</b> (" + stock.symbol + ") - $" + stock.price + "</p><input type='number' id='weightInput' placeholder='Weight %' min='1' max='100' style='width:80px;padding:5px;'><button onclick=\"addToPortfolio('" + stock.symbol + "', '" + stock.name.replace(/'/g, "\\'") + "')\">Add</button>";
}
        ...
        options: {
            ...
            plugins: {
                tooltip: {
                    callbacks: {
                        label: function(ctx) {
                            return ctx.raw.label + ': Risk ' + ctx.raw.x.toFixed(3) + '%, Return ' + ctx.raw.y.toFixed(2) + '%';
                        }
                    }
                },
